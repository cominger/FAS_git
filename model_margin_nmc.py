...
class FASNET(nn.Module):
    def __init__(self, batch_size ,feature_size, n_classes, pre_trained=False):
        # Network architecture
        super(FASNET, self).__init__()
        # self.feature_extractor = models.resnet18(num_classes=n_classes)
        ...
        self.fc = nn.Linear(feature_size, feature_size, bias=False)
        ...
        self.cms_loss = nn.MarginRankingLoss(margin=0.1)
        self.mlm_loss = nn.NLLLoss()
        ...
        if pre_trained:
            pretrained_model_file="pretrained_model.pkl"
    ...
    def update_representation(self, dataset, transform):
        ...
        for epoch in range(num_epochs):
            ...
            for k in range(self.n_classes):
                t_images = dataset.get_image_class(k)
                t_images_label = np.ones((t_images.shape[0])) * k
                t_images = numpy_2_dataset(t_images, t_images_label, transform)        
                k_mean = self.centroid_distance(t_images,transform)
                class_mean.append(k_mean)
            ...
            for i, (indices, images, labels) in enumerate(loader):
                ...
                np_label = labels.data.cpu().numpy()
                # cur_label_dist = class_mean[np_label]

                feature = self.forward(images)
                input_a = Variable(torch.from_numpy(class_mean)).cuda()
                # input_b = (cur_label_dist - np.sum(class_mean,0)) / (self.n_classes-1)
                # input_b = Variable(torch.from_numpy(input_b)).cuda()

                # correct_label_dist = F.pairwise_distance(feature, input_a, 2)
                # wrong_label_dist = F.pairwise_distance(feature, input_b, 2)
                # y_label = Variable(torch.FloatTensor(correct_label_dist.size()).fill_(1)).cuda()
                # loss += self.cms_loss(correct_label_dist, wrong_label_dist, y_label)

                mal_D_dist = feature.unsqueeze(1)-input_a #b*c*feature
                mal_D_dist = mal_D_dist.pow(2).sum(2)     #b*c
                # mal_D_dist = torch.exp(-1/2*mal_D_dist)   #b*c
                # loss +=self.mlm_loss(mal_D_dist,labels)
                loss +=self.cls_loss(mal_D_dist,labels)
