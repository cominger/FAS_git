...
def feature_output(dataloader,net, max_len):
    ...
    for cur_class in range(num_class):
        ...
        #w_p print
        plt.bar(center,hist,   align = 'center', width= width)
        plt.bar(center,wp_hist, align = 'center', width= width, alpha=0.5, color='red')
        title_name = 'class: '+ str(cur_class)+ ' total sample histogram ' + str(cur_value)
        plt.title(title_name)
        plt.ylabel('count')

        # plt.show()
        file_name='./progress_plot/class_'+str(cur_class)+'_percent.png'
        plt.savefig(file_name)
        plt.clf()
...
def main():
    ...
    pre_trained_path = './checkpoint/Clothing1M_deep_rest50_no_nmc_clean_dataset_imagenet_pretrained_sgd_.t7'
